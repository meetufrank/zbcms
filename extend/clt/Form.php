...
class Form extends controller{
    ...
    public function radio($info,$value){
        ...
        foreach($optionsarr as $key=>$r) {
            ...
            $parseStr .= '<input lay-filter="'.$field.'" name="'.$field.'" id="'.$id.'_'.$i.'" '.$checked.' value="'.$key.'" '.$validate.' type="radio" class="ace" title="'.$r.'" />';
            $i++;
        }
        return $parseStr;
    }
...
public function select_db($info,$value){
        ...
        $fields=$info['setup']['value'].",".base64_decode($info['setup']['name']);
         if(!empty($dbname)){
            $dbname= db($dbname);
            if(!empty($info['setup']['where'])){
               $query= $dbname->where(json_decode(base64_decode($info['setup']['where']),1));
            }else{
                $query= $dbname;
            } 
            if(!empty($fields)){
...
public function checkbox_db($info,$value){
    ...
    public function customize($info,$value) {
        
        return $this->view->display(base64_decode($info['setup']['content']));
