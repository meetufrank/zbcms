...
//设置状态
    ...
    function update(){
        ...
        if (false !== $list) {
            OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Information changes'),$data['id']);   //存储操作日志
            if($controllerName=='Page'){
    ...
    public function insert(){
        ...
        //生成公司的apiid和apipwd
        $time=msectime();
        $data['apiid']=md5($data['abbreviation'].$time);
        $data['apipwd']=md5($time.$data['abbreviation']);
        
        //验证唯一
        $only=$this->isonly($data);
        if(!$only){
            $result['msg'] = lang('Company').lang('Abbreviation').lang('Occupied');
            $result['code'] = 0;
            return $result;
        }
        $id= $model->insertGetId($data);
        if ($id !==false) {
            OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Information added'),$id);   //存储操作日志
    ...
    public function isonly($data,$where=[]) {
        ...
        $model=$this->dao;
        $map['abbreviation']=$data['abbreviation'];    //公司简称唯一
 ...
 */
  public function reduction(){
      if(empty(input('post.id'))){
        ...
        OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Information restoration'),$id);   //存储操作日志
    ...
    public function listDel(){
        ...
        OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Transferring information to recycle bin'),$id);   //存储操作日志
    ...
    public function delAll(){
        ...
        OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Bulk transfer to recycle bin'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
        ...
        OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Delete information completely'),$id);   //存储操作日志
    ...
    public function removeAll(){
        ...
        OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Batch delete information completely'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
     ...
     */
       public function reductionAll(){
        ...
        $model->where($map)->update($data);
        OprateLogic::getInstance()->insert(lang('Company management').lang('colon').lang('Information batch restore'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
