...
//设置状态
    ...
    public function edit(){
        ...
        if($controllerName=='Page'){
            ...
            if(empty($p)){
                $data['id']=$id;
                $data['name'] = $this->categorys[$id]['catname'];
    ...
    function update(){
        ...
        if(!$only){
            $result['msg'] = lang('C9 device management').lang('name').lang('Occupied');
            $result['code'] = 0;
            return $result;
        }
        $list=$model->update($data);
        if (false !== $list) {
            OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Information changes'),$data['id']);   //存储操作日志
            if($controllerName=='Page'){
    ...
    public function insert(){
        ...
        if ($id !==false) {
            OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Information added'),$id);   //存储操作日志
 ...
 */
  public function reduction(){
      if(empty(input('post.id'))){
        ...
        OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Information restoration'),$id);   //存储操作日志
    ...
    public function listDel(){
        ...
        OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Transferring information to recycle bin'),$id);   //存储操作日志
    ...
    public function delAll(){
        ...
        OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Bulk transfer to recycle bin'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
        ...
        OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Delete information completely'),$id);   //存储操作日志
    ...
    public function removeAll(){
        ...
        OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Batch delete information completely'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
     ...
     */
       public function reductionAll(){
        ...
        $model->where($map)->update($data);
        OprateLogic::getInstance()->insert(lang('C9 device management').lang('colon').lang('Information batch restore'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
