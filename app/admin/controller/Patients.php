    ...
    public function index(){
        if(request()->isPost()){
            ...
            if(in_array('catid',$field)){
               $map['p.catid']=array('in',$catid);
            }
            
            $map['p.deletetime']=0;
            $list = $model
                ->alias('p')
                ->join(config('database.prefix').'company c','p.company = c.id','left')
                ->field('p.*,c.name as companyname')
                ->where($map)
                ->order($order)
...
//设置状态
    ...
    function update(){
        ...
        if(!$only){
            $result['msg'] = lang('Patient portal').lang('name').lang('Occupied');
            $result['code'] = 0;
            return $result;
        }
        $list=$model->update($data);
        if (false !== $list) {
            OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Information changes'),$data['id']);   //存储操作日志
            if($controllerName=='Page'){
                $result['url'] = url("admin/category/index");
            }else{
                $result['url'] = url("admin/".$controllerName."/index",array('pid'=>$data['company'],'catid'=>$data['catid']));
            }
    ...
    public function add(){
        ...
        if(!$pid){
         ...
         $companyname=CompanyLogic::getInstance()->select('name',['id'=> $pid],1);
        
        $form=new Form();
        $this->assign('companyname',$companyname['name']);
        $this->assign ( 'form', $form );
    ...
    public function insert(){
        ...
        if ($id !==false) {
            OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Information added'),$id);   //存储操作日志
 ...
 */
  public function reduction(){
        ...
        OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Information restoration'),$id);   //存储操作日志
    ...
    public function listDel(){
        ...
        OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Transferring information to recycle bin'),$id);   //存储操作日志
    ...
    public function delAll(){
        if(empty(input('param.ids/a'))){
                ...
                $result['url'] = url('index',array('pid'=>input('post.pid'),'catid'=>input('post.catid')));
                return $result;
        }
        ...
        OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Bulk transfer to recycle bin'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
    ...
    public function listRemove(){
        ...
        OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Delete information completely'),$id);   //存储操作日志
    ...
    public function removeAll(){
        ...
        OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Batch delete information completely'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
     ...
     */
       public function reductionAll(){
        ...
        $model->where($map)->update($data);
        OprateLogic::getInstance()->insert(lang('Patient portal management').lang('colon').lang('Information batch restore'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
    ...
    public function listorder(){
        ...
        $data = input('post.');
        $model->update($data);
        $result = ['msg' => lang('Sorting success'),'url'=>url('index',array('pid'=>input('post.pid'),'catid'=>$catid)), 'code' => 1];
        return $result;
    }
